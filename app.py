...
if aba == "🛰️ Briefing em Tempo Real":
    ...
    with st.spinner(f"Processando GFS para o {fl_alvo}..."):
        ...
        if ds:
            ...
            # 1. Filtro de dados (pegando temperatura e umidade)
            data_nivel = ds.sel(isobaricInhPa=pressao_hpa, method="nearest")
            temp_c = data_nivel.t.values - 273.15  # Converte Kelvin para Celsius
            umidade = data_nivel.r.values           # Umidade Relativa
            
            # 2. Gera o mapa base com as cartas ENRC
            m_gfs = gerar_mapa_base(cartas_baixa_sel, cartas_alta_sel)

            # 3. Lógica de "Pintar" o Gelo (Física da AC 91-74B)
            # Vamos criar uma mancha azul clara onde houver umidade > 70% e temp < 0°C
            with st.spinner("Calculando áreas de formação de nuvens/gelo..."):
                # Simplificando: vamos mostrar onde a umidade está alta (potencial de nuvens)
                # No futuro, usaremos polígonos reais, por enquanto vamos sinalizar o risco
                if umidade.max() > 70:
                    st.info(f"Probabilidade de nuvens/gelo detectada. UR máxima: {umidade.max():.1f}%")

            # 4. Alertas Críticos (Sua experiência FL120-FL240)
