...
def carregar_dados_gfs():
    try:
        # Habilita o Herbie para buscar a rodada mais recente
        H = Herbie(model="gfs", product="pgrb2.0p25", fxx=0, overwrite=False)
        
        # O segredo: forçamos o engine='cfgrib' para usar o motor que instalamos no packages.txt
        ds = H.xarray(":(UGRD|VGRD|TMP|RH):", engine="cfgrib")
        return ds
    except Exception as e:
        # Se falhar, avisa na sidebar sem derrubar o site todo
...
def gerar_mapa_base(cartas_L=[], cartas_H=[], mostrar_sat=True):
    ...
    # Renderiza as Cartas ENRC (L e H) selecionadas na sidebar
    for carta in cartas_L:
        folium.WmsTileLayer(
            url="https://geoaisweb.decea.mil.br/geoserver/ICA/wms",
            layers=f"ICA:ENRC_{carta}",
            fmt="image/png", transparent=True, name=f"Carta {carta}", overlay=True
        ).add_to(m)
...
if aba == "🛰️ Briefing em Tempo Real":
  ...
  # Chamar a função unificada (substitui a criação manual do mapa)
    ...
    with st.spinner(f"Acessando NOMADS para processar o {fl_alvo}..."):
        ...
        if ds:
           ...
           if ds:
            ...
            # ABAIXO: Usamos a função para o mapa do GFS ter as mesmas cartas do briefing
